...
class StringExtension
{
	...
	public static inline function isAlpha(char:String):Bool
	{
		return (char.fastCodeAt(0) >= 'a'.code && char.fastCodeAt(0) <= 'z'.code)
			|| (char.fastCodeAt(0) >= 'A'.code && char.fastCodeAt(0) <= 'Z'.code);
	}
	...
	public static function pythonicFormat(orig:String, substitutions:Map<String, String>):String
	{
	...
	public static function toSnakeCase(orig:String):String
	{
		final buf:StringBuf = new StringBuf();
		var hasChars:Bool = false;

		for (char in orig)
		{
			if (!hasChars)
				buf.add(char.toLowerCase());
			else if (char.isUpperCase())
				buf.add("_" + char.toLowerCase());
			else
				buf.add(char);
			hasChars = true;
		}
	...
	public static function lastDelimitedPart(orig:String, delimiter:String):String
	{
		return orig.substr(orig.lastIndexOf(delimiter) + 1);
	}
}
