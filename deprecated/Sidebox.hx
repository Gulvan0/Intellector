...
class Sidebox extends Sprite
{
    ...
    {
        resignBtn = constructBtn(HALVED_BUTTON_WIDTH, RESIGN_BTN_ABORT_TEXT,  (e) -> {emit(ResignPressed);});
        offerDrawBtn = constructBtn(HALVED_BUTTON_WIDTH, OFFER_DRAW_BTN_TEXT, (e) -> {emit(OfferDrawPressed);});
        cancelDrawBtn = constructBtn(HALVED_BUTTON_WIDTH, CANCEL_DRAW_BTN_TEXT, (e) -> {emit(CancelDrawPressed);}, false);
        ...
        offerTakebackBtn = constructBtn(FULL_BUTTON_WIDTH, TAKEBACK_BTN_TEXT, (e) -> {emit(OfferTakebackPressed);});
        cancelTakebackBtn = constructBtn(FULL_BUTTON_WIDTH, CANCEL_TAKEBACK_BTN_TEXT, (e) -> {emit(CancelTakebackPressed);}, false);
        rematchBtn = constructBtn(FULL_BUTTON_WIDTH, REMATCH,  (e) -> {Networker.emitEvent(CreateDirectChallenge(opponentLogin, startSecs, secsPerTurn, playerIsWhite? "b" : "w"));}, false); //TODO: Check whether these values for 'color' are expected; derive opponentLogin and color from logins
    ...
    public function new(spectators:Bool, startSecs:Null<Int>, secsPerTurn:Null<Int>, playerLogin:String, opponentLogin:String, playerIsWhite:Bool) 
    {
        super();
        ...
        navigator = new MoveNavigator(type -> {emit(PlyScrollRequest(type));});
        exploreBtn = constructBtn(250, EXPLORE_IN_ANALYSIS_BTN_TEXT, (e) -> {emit(ExploreInAnalysisRequest);}, spectators);
        ...
        if (!spectators)
        {
            takebackRequestBox = buildRequestBox(box, TAKEBACK_QUESTION_TEXT, (e) -> {emit(AcceptTakebackPressed);}, (e) -> {emit(DeclineTakebackPressed);});
            drawRequestBox = buildRequestBox(box, DRAW_QUESTION_TEXT, (e) -> {emit(AcceptDrawPressed);}, (e) -> {emit(DeclineDrawPressed);});
        }
        ...
        box.addComponent(constructBtn(250, ANALYSIS_EXPORT_SIP,  (e) -> {emit(ExportSIPRequested);}));
